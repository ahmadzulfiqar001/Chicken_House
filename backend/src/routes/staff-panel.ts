import express from "express";
import { getRequestAuthUser, normalizeEmailInput, requireAuth } from "../auth";
import { db } from "../db";

const router = express.Router();

router.use(requireAuth);

const staffOnlyRoles = new Set(["manager", "rider", "staff"]);

const roleLabels: Record<string, string> = {
  manager: "Manager / Branch Supervisor",
  rider: "Rider / Delivery Staff",
  staff: "General Staff",
};

const resolveStaffMember = (authUser: ReturnType<typeof getRequestAuthUser>) => {
  if (!authUser) return null;

  if (authUser.staffMemberId) {
    return db.staff.find((member) => Number(member.id) === Number(authUser.staffMemberId)) ?? null;
  }

  return (
    db.staff.find((member) => String(member.email ?? "").toLowerCase() === authUser.email.toLowerCase()) ??
    null
  );
};

const ensureStaffAccess = (req: express.Request, res: express.Response) => {
  const authUser = getRequestAuthUser(req);

  if (!authUser || !staffOnlyRoles.has(authUser.role)) {
    res.status(403).json({ message: "This panel is available only for staff roles." });
    return null;
  }

  const staffMember = resolveStaffMember(authUser);

  if (!staffMember) {
    res.status(404).json({ message: "Linked staff profile was not found." });
    return null;
  }

  return { authUser, staffMember };
};

const addActivityLog = (
  staffMember: { id: number; name: string },
  role: string,
  action: string,
  detail: string,
) => {
  db.activityLogs.unshift({
    id: `ACT-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
    staffId: staffMember.id,
    staffName: staffMember.name,
    role,
    action,
    detail,
    createdAt: new Date().toISOString(),
  });
};

const buildInitials = (name: string) =>
  name
    .split(" ")
    .map((part) => part.trim()[0] ?? "")
    .join("")
    .slice(0, 2)
    .toUpperCase();

const syncStaffNameAcrossRecords = (staffId: number, nextName: string) => {
  [
    db.attendance,
    db.leaveRequests,
    db.payroll,
    db.shiftSchedules,
    db.performanceReviews,
    db.staffRequests,
    db.activityLogs,
  ].forEach((records) => {
    records.forEach((record) => {
      if (Number(record.staffId) === Number(staffId)) {
        record.staffName = nextName;
      }
    });
  });
};

const todayDate = () => new Date().toISOString().slice(0, 10);

const deriveAttendanceStatus = (staffMember: { shift?: string }, checkInTime: string) => {
  const shiftStartMap: Record<string, string> = {
    Morning: "09:00",
    Evening: "14:00",
    Night: "22:00",
  };

  const shiftStart = shiftStartMap[String(staffMember.shift ?? "")] ?? "09:00";
  const [shiftHour, shiftMinute] = shiftStart.split(":").map(Number);
  const [inHour, inMinute] = checkInTime.split(":").map(Number);
  const shiftTotal = shiftHour * 60 + shiftMinute;
  const inTotal = inHour * 60 + inMinute;

  return inTotal > shiftTotal + 5 ? "Late" : "Present";
};

const getVisibleNotices = (role: string, staffMemberId: number) => {
  const canSeeAllNotices = role === "manager";

  return db.staffNotices
    .filter((notice) => canSeeAllNotices || notice.audience.includes(role) || notice.audience.includes("staff"))
    .map((notice) => ({
      ...notice,
      seen: notice.seenBy.includes(staffMemberId),
    }));
};

const buildRoleTasks = (role: string, staffMemberId: number) => {
  if (role === "rider") {
    return db.orders
      .filter((order) => Number(order.assignedStaffId ?? 0) === Number(staffMemberId))
      .slice(0, 8)
      .map((order) => ({
        id: order.id,
        title: order.customer,
        status: order.status,
        subtitle: order.deliveryAddress || "Delivery address pending",
      }));
  }

  if (role === "staff") {
    return db.orders
      .filter((order) => {
        const assignedToCurrent =
          Number(order.assignedStaffId ?? 0) === Number(staffMemberId) ||
          (String(order.assignedRole ?? "") === "staff" && !Number(order.assignedStaffId ?? 0));

        return assignedToCurrent && order.status !== "Delivered" && order.status !== "Cancelled";
      })
      .slice(0, 8)
      .map((order) => ({
        id: order.id,
        title: order.items,
        status: order.status,
        subtitle: `${order.customer} | ${order.type}`,
      }));
  }

  return db.shiftSchedules
    .filter((shift) => Number(shift.staffId) === Number(staffMemberId))
    .slice(0, 5)
    .map((shift) => ({
      id: shift.id,
      title: shift.shiftType,
      status: shift.date,
      subtitle: `${shift.startTime || "--"} - ${shift.endTime || "--"}`,
    }));
};

router.get("/summary", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const { authUser, staffMember } = context;
  const attendance = db.attendance
    .filter((record) => Number(record.staffId) === Number(staffMember.id))
    .sort((left, right) => String(right.date).localeCompare(String(left.date)));
  const todayAttendance = attendance.find((record) => record.date === todayDate()) ?? null;
  const leaves = db.leaveRequests
    .filter((leave) => Number(leave.staffId) === Number(staffMember.id))
    .sort((left, right) => String(right.startDate).localeCompare(String(left.startDate)));
  const shifts = db.shiftSchedules
    .filter((shift) => Number(shift.staffId) === Number(staffMember.id))
    .sort((left, right) => String(left.date).localeCompare(String(right.date)));
  const payroll = db.payroll
    .filter((entry) => Number(entry.staffId) === Number(staffMember.id))
    .sort((left, right) => Number(right.year) - Number(left.year));
  const reviews = db.performanceReviews
    .filter((entry) => Number(entry.staffId) === Number(staffMember.id))
    .sort((left, right) => String(right.reviewDate).localeCompare(String(left.reviewDate)));
  const notices = getVisibleNotices(authUser.role, staffMember.id);
  const requests = db.staffRequests
    .filter((request) => Number(request.staffId) === Number(staffMember.id))
    .sort((left, right) => String(right.createdAt).localeCompare(String(left.createdAt)));
  const activity = db.activityLogs
    .filter((entry) => Number(entry.staffId) === Number(staffMember.id))
    .slice(0, 10);
  const tasks = buildRoleTasks(authUser.role, staffMember.id);
  const lateCount = attendance.filter((record) => record.status === "Late").length;
  const absentCount = attendance.filter((record) => record.status === "Absent").length;

  return res.json({
    roleLabel: roleLabels[authUser.role] ?? "Staff",
    staff: staffMember,
    todayAttendance,
    attendanceStats: {
      totalRecords: attendance.length,
      lateCount,
      absentCount,
      presentCount: attendance.filter((record) => ["Present", "Late"].includes(record.status)).length,
    },
    pendingLeaveCount: leaves.filter((leave) => leave.status === "Pending").length,
    currentShift: shifts.find((shift) => shift.date >= todayDate()) ?? shifts[0] ?? null,
    latestPayroll: payroll[0] ?? null,
    latestPerformance: reviews[0] ?? null,
    notices,
    requests,
    tasks,
    activity,
  });
});

router.get("/attendance", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const records = db.attendance
    .filter((record) => Number(record.staffId) === Number(context.staffMember.id))
    .sort((left, right) => String(right.date).localeCompare(String(left.date)));

  return res.json(records);
});

router.post("/attendance/mark", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const { authUser, staffMember } = context;
  const existing = db.attendance.find(
    (record) => Number(record.staffId) === Number(staffMember.id) && record.date === todayDate(),
  );

  if (existing) {
    return res.status(400).json({ message: "Attendance once submitted is locked for today." });
  }

  const currentTime = new Date().toTimeString().slice(0, 5);
  const status = deriveAttendanceStatus(staffMember, currentTime);
  const record = {
    id: `ATT-${Date.now()}`,
    staffId: staffMember.id,
    staffName: staffMember.name,
    date: todayDate(),
    clockIn: currentTime,
    clockOut: "",
    status,
    workHours: 0,
    notes: "",
    location: String(req.ip ?? ""),
    edited: false,
    adminApproval: "Locked",
  };

  db.attendance.unshift(record);
  addActivityLog(staffMember, authUser.role, "Attendance marked", `${staffMember.name} marked attendance at ${currentTime}.`);

  return res.status(201).json(record);
});

router.post("/attendance/corrections", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const { authUser, staffMember } = context;
  const request = {
    id: `REQ-${Date.now()}`,
    staffId: staffMember.id,
    staffName: staffMember.name,
    category: "Attendance Correction",
    subject: String(req.body?.subject ?? "Attendance correction request"),
    message: String(req.body?.message ?? "").trim(),
    targetDate: String(req.body?.targetDate ?? todayDate()),
    status: "Pending",
    createdAt: new Date().toISOString(),
    resolvedAt: "",
  };

  db.staffRequests.unshift(request);
  addActivityLog(staffMember, authUser.role, "Request sent", `${staffMember.name} requested attendance correction for ${request.targetDate}.`);

  return res.status(201).json(request);
});

router.get("/leaves", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const leaves = db.leaveRequests
    .filter((leave) => Number(leave.staffId) === Number(context.staffMember.id))
    .sort((left, right) => String(right.startDate).localeCompare(String(left.startDate)));

  return res.json(leaves);
});

router.post("/leaves", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const { authUser, staffMember } = context;
  const fromDate = String(req.body?.startDate ?? "");
  const toDate = String(req.body?.endDate ?? fromDate);
  const oneDay = 1000 * 60 * 60 * 24;
  const days = Math.max(1, Math.round((Date.parse(toDate) - Date.parse(fromDate)) / oneDay) + 1);

  const leave = {
    id: `LV-${Date.now()}`,
    staffId: staffMember.id,
    staffName: staffMember.name,
    leaveType: String(req.body?.leaveType ?? "Casual"),
    startDate: fromDate,
    endDate: toDate,
    days,
    reason: String(req.body?.reason ?? "").trim(),
    attachment: String(req.body?.attachment ?? ""),
    status: "Pending",
    approvedBy: "",
    approvedAt: "",
    rejectionReason: "",
  };

  db.leaveRequests.unshift(leave);
  addActivityLog(staffMember, authUser.role, "Leave applied", `${staffMember.name} applied for ${leave.leaveType} leave from ${fromDate} to ${toDate}.`);

  return res.status(201).json(leave);
});

router.delete("/leaves/:id", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const index = db.leaveRequests.findIndex(
    (leave) =>
      leave.id === req.params.id &&
      Number(leave.staffId) === Number(context.staffMember.id) &&
      leave.status === "Pending",
  );

  if (index === -1) {
    return res.status(404).json({ message: "Pending leave request not found." });
  }

  const [removed] = db.leaveRequests.splice(index, 1);
  addActivityLog(context.staffMember, context.authUser.role, "Leave cancelled", `${context.staffMember.name} cancelled pending leave ${removed.id}.`);
  return res.json({ message: "Pending leave request cancelled.", leave: removed });
});

router.get("/shifts", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const shifts = db.shiftSchedules
    .filter((shift) => Number(shift.staffId) === Number(context.staffMember.id))
    .sort((left, right) => String(left.date).localeCompare(String(right.date)));

  return res.json(shifts);
});

router.get("/tasks", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  return res.json(buildRoleTasks(context.authUser.role, context.staffMember.id));
});

router.get("/requests", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  return res.json(
    db.staffRequests
      .filter((request) => Number(request.staffId) === Number(context.staffMember.id))
      .sort((left, right) => String(right.createdAt).localeCompare(String(left.createdAt))),
  );
});

router.post("/requests", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const request = {
    id: `REQ-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`,
    staffId: context.staffMember.id,
    staffName: context.staffMember.name,
    category: String(req.body?.category ?? "General Request"),
    subject: String(req.body?.subject ?? "Staff request"),
    message: String(req.body?.message ?? "").trim(),
    status: "Pending",
    createdAt: new Date().toISOString(),
    resolvedAt: "",
  };

  db.staffRequests.unshift(request);
  addActivityLog(context.staffMember, context.authUser.role, "Request sent", `${context.staffMember.name} submitted a ${request.category} request.`);

  return res.status(201).json(request);
});

router.get("/notices", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  return res.json(getVisibleNotices(context.authUser.role, context.staffMember.id));
});

router.post("/notices/:id/seen", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const notice = db.staffNotices.find((entry) => entry.id === req.params.id);

  if (!notice) {
    return res.status(404).json({ message: "Notice not found." });
  }

  if (!notice.seenBy.includes(context.staffMember.id)) {
    notice.seenBy.push(context.staffMember.id);
    addActivityLog(context.staffMember, context.authUser.role, "Notice seen", `${context.staffMember.name} viewed notice ${notice.title}.`);
  }

  return res.json({ message: "Notice marked as seen." });
});

router.get("/profile", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const latestPayroll =
    db.payroll
      .filter((entry) => Number(entry.staffId) === Number(context.staffMember.id))
      .sort((left, right) => Number(right.year) - Number(left.year))[0] ?? null;
  const latestReview =
    db.performanceReviews
      .filter((entry) => Number(entry.staffId) === Number(context.staffMember.id))
      .sort((left, right) => String(right.reviewDate).localeCompare(String(left.reviewDate)))[0] ?? null;

  return res.json({
    staff: context.staffMember,
    latestPayroll,
    latestReview,
  });
});

router.patch("/profile", (req, res) => {
  const context = ensureStaffAccess(req, res);
  if (!context) return;

  const name = String(req.body?.name ?? "").trim();
  const email = normalizeEmailInput(String(req.body?.email ?? ""));
  const phone = String(req.body?.phone ?? "").trim();
  const address = String(req.body?.address ?? "").trim();
  const emergencyContact = String(req.body?.emergencyContact ?? "").trim();

  if (name.length < 2) {
    return res.status(400).json({ message: "Please enter a valid name." });
  }

  if (!email.includes("@")) {
    return res.status(400).json({ message: "Please enter a valid email address." });
  }

  const duplicateAccount = db.userAccounts.find(
    (account) =>
      account.id !== context.authUser.id &&
      String(account.email ?? "").toLowerCase() === email,
  );

  if (duplicateAccount) {
    return res.status(409).json({ message: "This email is already linked with another account." });
  }

  context.staffMember.name = name;
  context.staffMember.email = email;
  context.staffMember.phone = phone;
  context.staffMember.address = address;
  context.staffMember.emergencyContact = emergencyContact;

  const linkedAccount = db.userAccounts.find((account) => account.id === context.authUser.id);

  if (linkedAccount) {
    linkedAccount.name = name;
    linkedAccount.email = email;
    linkedAccount.phone = phone;
    linkedAccount.avatarInitials = buildInitials(name);
  }

  syncStaffNameAcrossRecords(context.staffMember.id, name);
  addActivityLog(context.staffMember, context.authUser.role, "Profile updated", `${name} updated his profile details.`);

  return res.json({
    message: "Profile updated successfully.",
    staff: context.staffMember,
  });
});

export default router;
