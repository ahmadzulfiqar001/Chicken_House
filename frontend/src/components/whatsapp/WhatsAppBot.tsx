  ...
  return (
    <>
      <motion.button
        ...
        className="fixed bottom-4 left-4 z-50 flex h-14 w-14 items-center justify-center rounded-full border-4 border-white bg-primary text-white shadow-2xl shadow-primary/40 sm:bottom-8 sm:left-6 sm:h-16 sm:w-16"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0, y: 80, scale: 0.7, transformOrigin: "bottom left" }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 80, scale: 0.7 }}
            className="fixed bottom-4 left-4 z-[60] flex h-[min(38rem,calc(100dvh-9rem))] w-[92vw] max-w-[430px] flex-col overflow-hidden rounded-[2.5rem] border border-white/10 bg-white shadow-2xl shadow-black/20 sm:bottom-8 sm:left-6 sm:h-[min(42rem,calc(100dvh-10rem))]"
          >
